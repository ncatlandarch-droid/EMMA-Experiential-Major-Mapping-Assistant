/**
 * Node.js batch generator for EMMA pre-recorded TTS clips.
 * Uses Gemini TTS API directly with GEMINI_API_KEY env var.
 * 
 * Usage: node generate-coaching-wavs.js
...
async function main() {
  ...
  for (let i = 0; i < SCRIPTS.length; i++) {
    ...
    const filename = `${script.id}.wav`;
    const filepath = path.join(OUTPUT_DIR, filename);

    process.stdout.write(`(${i + 1}/${SCRIPTS.length}) ${script.id}... `);

    try {
      const wavBuffer = await generateClip(script);
      fs.writeFileSync(filepath, wavBuffer);
      
      manifest.push({
        id: script.id,
        path: `assets/audio/coaching/caes-la/${filename}`
      });
  ...
  console.log(`\n═══════════════════════════════════════`);
  console.log(`Done! ${success} clips generated, ${errors} errors`);
