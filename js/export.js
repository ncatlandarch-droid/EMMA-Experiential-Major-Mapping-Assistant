...
Generated by EMMA · Developed by the University Student Success Office | NC Agricultural &amp; Technical State University | ncat.edu
  ...
  function buildMajorMapDocument(branding, timeline) {
    ...
    // Common job titles from jobTitles field, common employers from topEmployers
    const commonCareers = career.jobTitles || career.relatedFields || [];
    ...
    function buildChecklistRows() {
      ...
      categories.forEach(cat => {
        ...
        timeline.phases.forEach(phase => {
          // Show only experiential milestones in USSO map (Lisa's feedback: no courses)
          const milestones = phase.milestones.filter(m => m.category === cat.key && m.type !== 'course');
          const items = milestones.map(m => {
            return `<div class="ms-item">▪ ${m.label}</div>`;
          }).join('');
