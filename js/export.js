...
Generated by EMMA · Developed by the University Student Success Office | NC Agricultural &amp; Technical State University | ncat.edu
  ...
  function exportMajorMap() {
    ...
    if (typeof EMMA_MATRIX !== 'undefined') {
      EMMA_MATRIX.showToast('📋 USSO Major Map opened — use Print / Save as PDF', 'success');
    }
  }
  ...
  function buildMajorMapDocument(branding, timeline) {
    ...
    // Strip degree prefix for short display: "Bachelor of Science in Animal Science" → "Animal Science"
    const shortName = programName.replace(/^(Bachelor|Master)\s+of\s+\w+\s+in\s+/i, '');
    const collegeName = (branding.collegeName || 'College').replace(/\s*\(.*\)\s*$/, ''); // strip abbreviation
    const tagline = branding.tagline || '';
    const description = branding.programDescription || '';
    const career = branding.careerOutlook || {};

    // Program highlights: use branding.programHighlights if present, else derive from keySkills + certifications
    let highlights = branding.programHighlights;
    if (!highlights || !highlights.length) {
      highlights = [];
      if (career.keySkills && career.keySkills.length) {
        highlights.push(`Core competencies: ${career.keySkills.slice(0, 4).join(', ')}`);
      }
      if (career.certifications && career.certifications.length) {
        career.certifications.forEach(c => highlights.push(c));
      }
      if (career.relatedFields && career.relatedFields.length) {
        highlights.push(`Career pathways in ${career.relatedFields.slice(0, 3).join(', ')}`);
      }
      if (career.medianSalary) {
        highlights.push(`Median salary: ${career.medianSalary}`);
      }
      if (!highlights.length) {
        highlights = ['Hands-on experiential learning', 'Faculty mentorship', 'Career-ready graduates'];
      }
    }

    // Common careers from relatedFields, common employers from topEmployers
    const commonCareers = career.relatedFields || [];
    const commonEmployers = career.topEmployers || [];
    ...
    const categories = [
      { key: 'Purpose',     emoji: '🔍', title: 'DISCOVER & DEMONSTRATE PURPOSE', sub: 'Connecting passion to purpose' },
      { key: 'Communities', emoji: '🤝', title: 'CULTIVATE COMMUNITIES',          sub: 'Building networks and belonging' },
      { key: 'LocalGlobal', emoji: '🌍', title: 'ENGAGE LOCALLY & GLOBALLY',      sub: 'Applying knowledge beyond campus' },
      { key: 'Identity',    emoji: '💼', title: 'DEVELOP PROFESSIONAL IDENTITY',  sub: 'Gaining credentials and achieving career readiness' }
    ];

    const phaseLabels = [
      { year: 'YEAR 1 – FRESHMAN',  theme: 'EXPLORE', sub: 'Orientation & Discovery' },
      { year: 'YEAR 2 – SOPHOMORE', theme: 'ENGAGE',  sub: 'Leadership & Skill Building' },
      { year: 'YEAR 3 – JUNIOR',    theme: 'DEVELOP', sub: 'Internships & Networking' },
      { year: 'YEAR 4 – SENIOR',    theme: 'LAUNCH',  sub: 'Capstone & Career Search' }
    ];
    ...
    function buildChecklistRows() {
      ...
      categories.forEach(cat => {
        rows += `<tr>
          <td class="cat-label" style="background:${BLUE}; color:white; font-weight:700; text-align:center; width:180px; vertical-align:top; padding:10px 8px;">
            <div style="font-size:20px; margin-bottom:4px;">${cat.emoji}</div>
            <div style="font-size:10px; line-height:1.3; text-transform:uppercase; letter-spacing:0.5px;">${cat.title}</div>
            <div style="font-size:8px; font-weight:400; font-style:italic; margin-top:3px; opacity:0.85;">${cat.sub}</div>
          </td>`;

        timeline.phases.forEach(phase => {
          const milestones = phase.milestones.filter(m => m.category === cat.key);
          const items = milestones.map(m => {
            const courseCode = m.courseRef ? m.courseRef.split(' — ')[0] : '';
            return `<div class="ms-item">☐ ${m.label}${courseCode ? ` <span class="ms-course">(${courseCode})</span>` : ''}</div>`;
          }).join('');
          rows += `<td class="ms-cell">${items || '<div class="ms-item" style="color:#aaa; font-style:italic;">See advisor</div>'}</td>`;
        });
...
<head>
  <meta charset="UTF-8">
  <title>Experiential Major Map — ${shortName}</title>
  <style>
    /* ── Base ── */
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: Calibri, Arial, Helvetica, sans-serif; color: #222; background: #fff; }
    ...
    /* ── Page Container (screen preview) ── */
    .page {
      width: 10in; min-height: 7.5in; margin: 20px auto;
    ...
    /* ════════════ PAGE 1 ════════════ */
    .p1-header {
      background: ${BLUE}; color: rgba(255,255,255,0.7);
      font-size: 8px; padding: 4px 20px; text-align: right;
    }
    .p1-title-band {
      background: ${BLUE}; color: white; padding: 14px 28px 10px;
    }
    .p1-title-band h1 {
      font-size: 22px; font-weight: 800; letter-spacing: 2px;
      margin-bottom: 2px; text-transform: uppercase;
    }
    .p1-title-band .degree { font-size: 16px; font-weight: 600; margin-bottom: 1px; }
    .p1-title-band .college { font-size: 10px; font-weight: 400; opacity: 0.85; }
    .p1-tagline {
      background: ${GOLD}; color: ${BLUE}; font-weight: 700;
      font-size: 10px; padding: 8px 28px; line-height: 1.4;
    }

    .p1-body { display: flex; padding: 14px 28px 10px; gap: 24px; }
    .p1-left { flex: 0 0 58%; }
    .p1-right { flex: 1; }
    ...
    .desc-text { font-size: 10px; line-height: 1.5; color: #333; }
    .bullet-list { list-style: none; padding: 0; }
    .bullet-list li {
      font-size: 9.5px; line-height: 1.45; padding: 2px 0 2px 14px;
      position: relative; color: #333;
    }
    .bullet-list li::before {
      content: '▸'; position: absolute; left: 0; color: ${GOLD}; font-weight: 700;
    }

    .p1-orgs-band {
      background: ${BLUE}; color: white; padding: 10px 28px;
    ...
    .p1-orgs-band .org-list { font-size: 9px; line-height: 1.5; opacity: 0.9; }

    /* ════════════ PAGE 2 ════════════ */
    .p2-header-band {
      background: ${BLUE}; color: white; padding: 14px 28px 10px; text-align: center;
    }
    .p2-header-band h1 {
      font-size: 22px; font-weight: 800; letter-spacing: 2px;
    }
    ...
    .p2-grid th .year-label { font-size: 10px; display: block; }
    .p2-grid th .theme-label { font-size: 9px; font-weight: 700; display: block; margin-top: 2px; }
    .p2-grid th .theme-sub { font-size: 7.5px; font-weight: 400; display: block; font-style: italic; }
    .p2-grid th.corner { background: ${BLUE}; }
    ...
    .ms-course { color: ${BLUE}; font-weight: 600; font-size: 8px; }

    .p2-footer {
      background: ${BLUE}; color: rgba(255,255,255,0.7);
      font-size: 8px; padding: 5px 20px; text-align: center;
      position: absolute; bottom: 0; left: 0; right: 0;
    }

    /* ── Print button ── */
    .print-bar {
      text-align: center; padding: 18px; background: #f0f0f0;
    }
    .print-bar button {
      padding: 12px 36px; background: ${BLUE}; color: white;
...
<body>
  ...
  <!-- ════════════════════════════════════════ PAGE 1 ════════════════════════════════════════ -->
  <div class="page">
    <div class="p1-header">© 2026 | University Student Success Office | ncat.edu</div>

    <div class="p1-title-band">
    ...
    <div class="p1-body">
      <!-- LEFT COLUMN -->
      <div class="p1-left">
        <div class="section-head">WHAT IS ${shortName.toUpperCase()}?</div>
        <p class="desc-text">${description}</p>

        <div class="section-head">PROGRAM HIGHLIGHTS</div>
      ...
      <!-- RIGHT COLUMN -->
      <div class="p1-right">
        <div class="section-head">WHAT DO ${shortName.toUpperCase()} MAJORS DO AFTER GRADUATION?</div>

        ${commonCareers.length ? `
        <div class="section-head" style="font-size:9px; border-bottom:1px solid ${GOLD}; margin-top:8px;">Common Careers</div>
        <ul class="bullet-list">
          ${commonCareers.map(c => `<li>${c}</li>`).join('')}
        </ul>` : ''}

        ${commonEmployers.length ? `
        <div class="section-head" style="font-size:9px; border-bottom:1px solid ${GOLD}; margin-top:8px;">Common Employers & Industries</div>
        <ul class="bullet-list">
          ${commonEmployers.map(e => `<li>${e}</li>`).join('')}
        </ul>` : ''}
      </div>
    </div>
  ...
  <div class="page">
    ...
    <table class="p2-grid">
      <thead>
        <tr>
          <th class="corner" style="width:180px;"></th>
          ${phaseLabels.map(pl => `
          <th>
            <span class="year-label">${pl.year}</span>
            <span class="theme-label">❖ ${pl.theme} ❖</span>
    ...
    <div class="p2-footer">© 2026 | University Student Success Office | ncat.edu</div>
  </div>
